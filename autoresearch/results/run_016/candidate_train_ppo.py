...
class CnnActorCritic(nn.Module):
    ...
    def __init__(self, obs_shape, action_dim: int, hp: dict = None):
        ...
        # Temporarily create features to calculate n_flatten before defining the full sequential
        temp_features = nn.Sequential(
        ...
        with torch.no_grad():
            n_flatten = temp_features(torch.zeros(1, c, h, w)).shape[1]
        ...
        self.features = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Flatten(),
            nn.LayerNorm(n_flatten) # ADDED: LayerNorm after flattening convolutional features
        )
