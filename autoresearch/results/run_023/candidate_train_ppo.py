...
class CnnActorCritic(nn.Module):
    ...
    def __init__(self, obs_shape, action_dim: int, hp: dict = None):
        ...
        # Temporary sequential to calculate n_flatten before building the final features module
        temp_conv_extractor = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Flatten(), # Flatten to get the size for LayerNorm
        )
        with torch.no_grad():
            n_flatten = temp_conv_extractor(torch.zeros(1, c, h, w)).shape[1]
        ...
        self.features = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Flatten(),
            nn.LayerNorm(n_flatten), # ADDED: LayerNorm after conv and flatten
        )
