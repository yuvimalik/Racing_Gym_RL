...
class CnnActorCritic(nn.Module):
    ...
    def __init__(self, obs_shape, action_dim: int, hp: dict = None):
        ...
        if action_dim >= 1:
            # Change: Make initial steer std tighter to reduce initial erratic behavior
            # Original: log_std_init[0] = 0.0 (std=1.0)
            log_std_init[0] = -0.5 # Initial std for steer (std = exp(-0.5) approx 0.6)
        if action_dim >= 3:
