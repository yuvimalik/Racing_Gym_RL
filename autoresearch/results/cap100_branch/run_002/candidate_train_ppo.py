...
HYPERPARAMS = {
    ...
    "steer_min_log_std": -1.0, # Specific min log_std for steer raw action (changed from -0.5)
    "steer_max_log_std": -0.2, # Specific max log_std for steer raw action (changed from 0.0)
}
...
class CnnActorCritic(nn.Module):
    ...
    def __init__(self, obs_shape, action_dim: int, hp: dict = None):
        ...
        if action_dim >= 1:
            log_std_init[0] = -0.2 # Initial std for steer (consistent with new max)
        if action_dim >= 3:
