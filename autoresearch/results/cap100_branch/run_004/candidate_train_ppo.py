...
HYPERPARAMS = {
    ...
    "vf_coef": 0.25,
    "max_grad_norm": 0.5,
    # Reverted: Removed steer_min_log_std/steer_max_log_std. Using general for all.
    "min_log_std": -2.0, # CHANGED: Reduced min_log_std to allow for more precise actions.
    "max_log_std": 0.5,
    # NEW: Huber Loss delta for value function
    "huber_delta": 1.0, 
}
...
class CnnActorCritic(nn.Module):
    ...
    def __init__(self, obs_shape, action_dim: int, hp: dict = None):
        ...
        # Reverted: Simplified log_std initialization.
        # All actions now use a general initial log_std of -0.5 (std ~ 0.6).
        log_std_init = torch.full((action_dim,), -0.5) 
        self.log_std = nn.Parameter(log_std_init)

        # Reverted: Now using general min/max log_std for all actions
        self.min_log_std = float(hp.get("min_log_std", -1.0))
        self.max_log_std = float(hp.get("max_log_std", 0.5))
        
        # Reverted: Removed initial biases for throttle/brake action means
        # The agent will learn these biases from scratch.
    ...
    def get_raw_dist_and_value(self, obs: torch.Tensor):
        ...
        # Reverted: Simplified log_std clamping. Apply global bounds to all actions.
    ...
    def _ppo_update(self, buf: dict) -> dict:
        ...
        for epoch in range(self.hp["n_epochs"]):
            ...
            for start in range(0, n_samples, batch_size):
                ...
                # NEW: Use Huber loss for the value function to improve stability
                vf_loss = F.huber_loss(values, ret_b, delta=self.hp["huber_delta"])
