...
class RewardShapingWrapper(gym.Wrapper):
    ...
    def __init__(self, env, reward_config):
        ...
        self.exit_speed_bonus_scale = float(reward_config.get('exit_speed_bonus_scale', 0.0))
        self.exit_speed_bonus_window = int(reward_config.get('exit_speed_bonus_window', 6))
        ...
        # Soft pre-collision penalty for "too close and closing in" situations before a collision is
        # registered. Keep default off for backward-compatible behavior.
        self.precollision_penalty_scale = float(marl_cfg.get("precollision_penalty_scale", 0.0))
        self.precollision_distance_threshold = float(
            marl_cfg.get("precollision_distance_threshold", max(0.5, self.collision_distance_threshold * 1.5))
        )
        self.precollision_min_closing_speed = float(marl_cfg.get("precollision_min_closing_speed", 0.0))
        ...
        self._prev_is_sharp_turn = np.zeros(0, dtype=np.bool_)
        self._post_corner_steps = np.zeros(0, dtype=np.int32)
        ...
        self._needs_track_context = (
            ...
            or self.exit_speed_bonus_scale != 0.0
    ...
    def _reset_agent_buffers(self, n_agents: int):
        ...
        self._active_collision_pairs = set()
        self._prev_is_sharp_turn = np.zeros(self._num_agents, dtype=np.bool_)
        self._post_corner_steps = np.zeros(self._num_agents, dtype=np.int32)
    ...
    def _compute_precollision_penalties(self, positions, velocity_vecs):
        penalties = np.zeros(self._num_agents, dtype=np.float32)
        nearest_risk_distance = np.full(self._num_agents, np.inf, dtype=np.float32)
        if (
            self._num_agents <= 1
            or not self.multi_agent_enabled
            or self.precollision_penalty_scale == 0.0
            or self.precollision_distance_threshold <= 0.0
        ):
            return penalties, nearest_risk_distance

        thr = float(max(self.precollision_distance_threshold, 1e-6))
        for i in range(self._num_agents):
            for j in range(i + 1, self._num_agents):
                ...
                if distance > thr:
                    continue
                nearest_risk_distance[i] = min(nearest_risk_distance[i], distance)
                nearest_risk_distance[j] = min(nearest_risk_distance[j], distance)
                ...
                # If we are already in collision proxy logic, let collision penalties handle it.
                collision_proxy = (
                ...
                closing_excess = max(0.0, closing_speed - self.precollision_min_closing_speed)
                if closing_excess <= 0.0:
                    continue
                closeness = max(0.0, 1.0 - (distance / thr))
                penalty = -self.precollision_penalty_scale * closeness * closing_excess
                penalties[i] += penalty
                penalties[j] += penalty

        return penalties.astype(np.float32), nearest_risk_distance
    ...
    def step(self, action):
        ...
        comp_precollision = np.zeros(self._num_agents, dtype=np.float32)
        collision_counts = np.zeros(self._num_agents, dtype=np.int32)
        collision_relative_speed = np.zeros(self._num_agents, dtype=np.float32)
        nearest_risk_dist = np.full(self._num_agents, np.inf, dtype=np.float32)
        ...
        if self.multi_agent_enabled and self._num_agents > 1:
            ...
            comp_precollision, nearest_risk_dist = self._compute_precollision_penalties(positions, velocity_vecs)
        ...
        comp_exit_speed = np.zeros(self._num_agents, dtype=np.float32)
        if self.exit_speed_bonus_scale != 0.0:
            prev_speed = np.nan_to_num(self._prev_speed, nan=speeds)
            speed_gain = np.maximum(0.0, speeds - prev_speed)
            just_exited_corner = np.logical_and(self._prev_is_sharp_turn, np.logical_not(is_sharp_turn))
            self._post_corner_steps = np.where(just_exited_corner, 1, self._post_corner_steps)
            active_exit_window = self._post_corner_steps > 0
            if self.exit_speed_bonus_window > 0:
                active_exit_window = np.logical_and(
                    active_exit_window,
                    self._post_corner_steps <= self.exit_speed_bonus_window,
                )
            comp_exit_speed = self.exit_speed_bonus_scale * speed_gain * active_exit_window.astype(np.float32)
            self._post_corner_steps = np.where(active_exit_window, self._post_corner_steps + 1, 0)
            self._post_corner_steps = np.where(is_sharp_turn, 0, self._post_corner_steps)
        ...
        self._prev_is_sharp_turn = is_sharp_turn.astype(np.bool_)
        ...
        for idx, agent_info in enumerate(info_list):
            ...
            if not self._training_mode:
                ...
                agent_info["telemetry/precollision_distance"] = float(nearest_risk_dist[idx]) if np.isfinite(nearest_risk_dist[idx]) else -1.0
                ...
                agent_info["rewards/exit_speed"] = float(comp_exit_speed[idx])
                ...
                agent_info["rewards/precollision"] = float(comp_precollision[idx])
...
class TelemetryCallback(BaseCallback):
    ...
    def __init__(self, verbose=0):
        ...
        self.metric_keys = [
            ...
            "telemetry/precollision_distance",
            "events/offtrack",
            "events/stuck",
            "events/collision",
            "events/overtake",
